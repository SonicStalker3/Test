        ...
        foreach (KeyValuePair<string, InputAction> action in actionsList)
        {
            ...
            if (ignoreLayers.Any(n => key[0] != n))
            {
                if (currentKey != key[0])
                {
                    contentSize += size;
                    c = 0;
                    var labelObj = Instantiate(_labelPrefab,transform);
                    var label =labelObj.GetComponent<TextMeshProUGUI>();
                    label.text = key[0]; // Set the label text to "GamePlay"
                    label.fontSize = 18;
                ...
                bindBtnText.text = action.Value.bindings.FirstOrDefault(n => n.groups == "Keyboard&Mouse").effectivePath;
                foreach (var x in action.Value.bindings.Where(n => n.groups == "Keyboard&Mouse"))
                {
                    Debug.Log(x.effectivePath);
                }
